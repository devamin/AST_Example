/*
 * 	Assignement 3 : Simple Optimisation
 *   
 *   
...
class Test4 {
	...
	private FuncNode funcNode = new FuncNode("test4", "a");

	
	private List<Node> getListOfInstructionBeforeOptimisation(){
		ArrayList<Node> listOfInst = new ArrayList<>();
		//For1
		ForStatement forStatement = new ForStatement(new DeclarationStatement("i"), new Literal(0),Reference.get("a"), null);
			//block of for1
			ArrayList<Node> block1 = new ArrayList<>();
			
			block1.add(new ForStatement(new DeclarationStatement("i2"), new Literal(0), Reference.get("a"),new ArrayList<Node>()));
		//set block to for1
			forStatement.setBlock(block1);
		//return a+1;
		ReturnStatement returnStatement = new ReturnStatement(
					new AdditionOperator(Reference.get("a"), new Literal(1))
				);
				
		listOfInst.add(forStatement);
